...
class EmailService {
    ...
    async sendEmail({ to, subject, html, attachments = [] }) {
        try {
            ...
            const fromName = process.env.EMAIL_FROM_NAME || 'Basha Food';
    ...
    async sendPasswordResetEmail(email, name, resetLink) {
        ...
        const html = `
            <div style="font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; max-width: 600px; margin: 0 auto; padding: 0; border: 1px solid #eee; border-radius: 12px; overflow: hidden; background-color: #ffffff;">
                <div style="padding: 30px 20px; text-align: center;">
                    <img src="${logoUrl}" alt="Basha Food" style="max-width: 180px; height: auto; display: block; margin: 0 auto;">
                </div>
                <div style="padding: 30px; color: #1e293b; text-align: center; border-top: 1px solid #f1f5f9;">
                    ...
                    <p style="font-size: 16px; line-height: 1.6; color: #475569; text-align: left;">We received a request to reset the password for your Basha Food account. If you didn't make this request, you can safely ignore this email.</p>
                ...
                <div style="background-color: #f8fafc; padding: 30px; text-align: center; border-top: 1px solid #eee;">
                    <p style="font-size: 14px; color: #64748b; margin: 0;">&copy; 2024 Basha Food. All rights reserved.</p>
                </div>
            </div>
        `;
        ...
        return await this.sendEmail({
            to: email,
            subject: 'Reset Your Password - Basha Food',
            html
        });
    }
}
